...
**Тялото на крепостта** се състои от начало `|`, среда "(празно място)" и край `|`. **Средaта** от празно място е с големина `2 * n - 2`. Броят на **редовете** за стени, можем да определим от дадените ни примери - `n -3`
...
Структурата на **основата на крепостта** е еднаква с тази на **покрива**. Съставена е от **две кули** и **междинна част**. **Кулата** има начало `\\`, среда `_` и край `/`.
...
За да направим **тялото на пеперудата**, можем отново да преизползваме **променливата** `halfRowSize` и да отпечатаме на конзолата точно **един** ред. Структурата на тялото е с начало "(празно място)", среда `@` и край "(празно място)".

![code](/assets/chapter-6-images/butterfly/image-05.png)

Остана да изпечатаме на конзолата и долното крило, което е еднакво с горното крило.
...
Трябва да създадем и втора **променлива**, която пази **стойността** на **средата на началния ред** и е с големина `2 * n + 1`.
...
**Долната част** на фигурата в която знака се **смалява** можем да направим, като отново създадем **цикъл**, който да се завърти `n` брой пъти. Струкурата на един ред е начало `.` + `\\\\`, среда `_` и край `//` + `.`. Броят на **точките** при първата интерация на цикала трябва да е 0 и на всяка следваща да се **увеличава** с едно. Следователно можем да кажем, че големината на **точките в долната част от фигурата** е равна на `i`.

![code](/assets/chapter-6-images/stop/image-08.png)
		
За да работи нашата програма правилно трябва на всяка интерация от **цикъла** да **намаляваме** броя на `_` с две.
