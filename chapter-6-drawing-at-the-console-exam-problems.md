...
**Стените в крепостта** се състоят от начало `|`, среда " " и край `|`. **Средaта** от празно място е с големина `2 * n - 2`. Броят на **редовете** за стени, можем да определим от дадените ни примери - `n -3`
...
От примерите можем да забележим, че на четен ред имаме - начало `*`, среда `\\ /` и край `*`, а на нечетен - начало `-`, среда `\\ /` и край `-`. Следователно трябва да направим `if else` проверка дали е четен реда и съответно да изпечатаме един от двата типа редове.
