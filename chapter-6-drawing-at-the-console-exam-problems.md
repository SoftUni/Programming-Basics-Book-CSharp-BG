...
Можем да разделим фигурата на **3 части** - горна, средна и долна. **Горната част** се състои от две подчасти - начален ред и тяло на стрелката. Броят на **външните точки** в началния ред и в тялото на стрелката са `(n - 1) / 2`. Тази стойност можем да запишем в **променлива** `outerDots`. 
...
Тъй като `new string` не може да съедини символ 0 пъти, **цъкъла**,  който ще направим, трябва да се завърти `n - 2` пъти и отделно да изпечатаме последния ред от фигурата. На всяка итерация `outerDots` се увеличава с 1, а `innerDots` намалява с 2.
