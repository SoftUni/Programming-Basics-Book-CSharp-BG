...
От примерите можем да забележим, че на четен ред имаме - начало `*`, среда `\\ /` и край `*`, а на нечетен - начало `-`, среда `\\ /` и край `-`. За да направим проверка дали е четен реда можем да напишем `if else` конструкция с условие `i % 2`. Ако условието е вярно тогава трябва да изпечатаме `***\\ /***`, ако не е вярно `---\\ /---`.
