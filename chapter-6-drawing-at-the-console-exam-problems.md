...
За да нарисуваме предпоследния ред, който е част от основата, трябва да изпечатаме начало `|`, среда "(празно място)" + `_` + "(празно място)" и край `|`. За да направим това, можем да използваме отново вече декларираните от нас променливи `colSize` и `midSize`, защото от примерите виждаме, че са равни на `_` в покрива.
...
След като вече сме декларирали двете промениви, можем да изпечатаме на конзолата началния ред.
...
Средната част от фигурата има начало `//` + `_`, среда `STOP` и край `_` + `\\\\`. Големината на `_` е `(underscopes - 5) / 2`.
...
За да работи нашата програма правилно трябва на всяка интерация от цикъла да намаляваме `_` с две.
