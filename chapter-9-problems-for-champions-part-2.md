...
Забележки:
* Използваме `decimal` тип за съхранение от резултата от нашето уравнение, за да избегнем всякакви проблем с точността, които съпътстват `float` и `double` типовете.
* Оператора по подразбиране е `+`, за да може още първото срещнато число да бъде събрано с резултата ни.

След като вече имаме началните си променливи, е добре да помислим върху това каква ще основната структура на нашата програма. От условието разбираме, че всеки израз завършва с `=`, т.е. ще трябва да четем и обработваме символи, докато не срещнем `=`. Това е точното изписване на `while` цикъл.
