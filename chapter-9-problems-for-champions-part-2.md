...
Пазаруването засвършва, когато се получи командата **"mall.Exit"**. Когато това стане, трябва да принтирате броя на извършени покупки и парите, които са останали на Лина. 
...
|----------------------------|------------------------------------|------------------------------------------------------------------------------------------------------------------|
| 110<br>mall.Enter<br>d<br>mall.Exit | 1 purchases. Money left: 80.00 lv. | ‘d’ има ASCII код 100.‘d’ e mалка буква е и за това Лина получава 70% отстъпка.100% – 70% = 30.110 – 30 = 80 лв. |
...
Ще разделим решението на задачата на три основни части - обработка на входа, алгоритъм на решаване и форматиране на изхода. Нека разгледаме всяка една част в детайли.
...
* т.к. буквите в азбуката имат ред, можем да използваме следната проверка `action >= 'A' && action <= 'Z'`, за да проверим дали нашият символ се намира в интервала от големи букви.
...
Забележка: `purchases` е `int` променлива, в която държим броя на всички покупки.

Т.к. всички други проверки са много сходни с първата, смятаме, че читателя не би трябвало да изпита проблем при тяхната имплементация.
...
Бони е много мощна вещица. Тъй като силата на природата не е достатъчна, за да се бори успешно с вампири и върколаци, тqя започнала усвоява силата на Изразите.
Израза е много труден за усвояване, т.к. заклинанието разчита на способността за **бързо решаване на математически изрази**. За използване на Израз заклинание, вещицата трябва да знае резултата от математическия израз предварително.
...
За решаване на задачата ще изпозлваме втория вариант.

![code](assets/chapter-9-2-images/02.X-expression-03.png)

##### Алгоритъм за решаване

За целите на нашата задача, ще имаме нужда от две променливи: една променлива, в която ще пазим текущия резултат, и още една променлива, в която ще пазим текущия оператор от нашия израз.

![code](assets/chapter-9-2-images/02.X-expression-04.png)

Относно кода по-горе нека поясним, че използваме тип `decimal` за съхранение на резултата на нашето уравнение, за да избегнем всякакви проблем с точността, които съпътстват типовете `float` и `double`. Оператора по подразбиране е `+`, за да може още първото срещнато число да бъде събрано с резултата ни.
...
#### Тестване в Judge системата
https://judge.softuni.bg/Contests/Practice/Index/519#1
...
Преди да започнем писането на алгоритъма за решаване на нашия проблем, е хубаво да декларираме флаг, който да указва дали е намерено решение:

![code](assets/chapter-9-2-images/03.Bulls-and-cows-04.png)

Ако след приключването на нашия алгоритъм, този флаг е още `false`, тогава ще принтираме `No` на конзолата, както е указано в условието.

![code](assets/chapter-9-2-images/03.Bulls-and-cows-05.png)

Нека започнем да размишляваме над нашия проблем. Това от което имаме нужда е да анализираме всички числа от `1111` до `9999`, без тези които съдържат нули в себе си. (т.е. `9011`, `3401` и т.н. са невалидни). И какъв е най-лесния начин да генерираме всички тези числа? С вложени цикли. Т.е. имаме 4 цифрено число, ще имаме 4 вложени цикъла, като всеки един от тях ще генерира отделна цифра от нашето число за тестване.

![code](assets/chapter-9-2-images/03.Bulls-and-cows-06.png)

Благодарение на тези цикли, имаме достъп до всяка една цифра на всички числа, които трябва да проверим. Следващата ни стъпка е да разделим секретното число на цифри. Това може да стане много лесно, чрез комбинация от целочислено и модулно деление.

![code](assets/chapter-9-2-images/03.Bulls-and-cows-07.png)

И имаме последните две стъпки преди да започнем да анализираме колко крави и бикове има в дадено число:
1. Да декларираме `counter` променливи във вложените ни цикли за да броим кравите и биковете за текущото число.
2. Да направим копия на цифрите на текущото число, което ще анализираме, за да предотвратим проблеми с работата на вложите цикли ако правим промени по тях.

![code](assets/chapter-9-2-images/03.Bulls-and-cows-08.png)

Вече сме готови да започнем да анализираме генерираните числа. Каква логика можем да използваме? Най-елементарния начин да проверим колко крави и бикове има в едно число е, чрез поредица от `if-else` проверки. Да, не е най-оптималния начин, но с цел да не използваме знания извън пределите на тази книга, ще изберем този подход.

От какви проверки имаме нужда?
1. Проверката за бикове е елементарна - проверяваме дали първата цифра от генерираното число е еднаква със същата цифра от секретното число. Премахваме проверените цифри с цел да избегнем повторения на бикове и крави.

![code](assets/chapter-9-2-images/03.Bulls-and-cows-09.png)

Повтаряме действието за втората, третата и четвърта цифра. 

2. Проверката за крави, можем да направи по следния начин:
    - Проверяваме дали първата цифра от генерираното число съвпада с втората, третата или четвърта цифра на секретното число. Примерна имплементация:
    ![code](assets/chapter-9-2-images/03.Bulls-and-cows-10.png)

    - Проверяваме дали втората цифра от генерираното число съвпада с първата, третата или четвърта цифра на секретното число.
    - Проверяваме дали третата цифра от генерираното число съвпада с първата, втората или четвърта цифра на секретното число.
    - Проверяваме дали четвърта цифра от генерираното число съвпада с първата, втората или третата цифра на секретното число.

След като приключим всички проверка, ни остава единствено да проверим дали биковете и кравите в текущото генерирано число съвпадат със желаните бикове и крави, прочетени от конзолата. Ако това е така, принтираме текущото число на конзолата.

![code](assets/chapter-9-2-images/03.Bulls-and-cows-11.png)

#### Тестване в Judge системата
https://judge.softuni.bg/Contests/Practice/Index/519#2
