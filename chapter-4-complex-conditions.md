...
```
Програмата ще се изпълни по следния начин: започва проверката от **`а`**, отчита, че има стойност `true` и проверява **`b`**. След като е отчелa, че **`a`** и **`b`** връщат `true`, проверява следващия аргумент. Стига до **`c`** и веднага вижда, че променливата има стойност `false`. Аргументът **`c`** бъдейки `false`, целия израз бива изчислен до стойност false, независимо от стойността на **`d`**. За това проверката на **`d`** се прескача и целия израз бива изчислен като `false`.
