...
### Насоки и подсказки
Нека първо разгледаме условието на задачата. Както при всяка една задача по програмиране, е важно да имаме идея за това как тя трябва да бъде решена преди да започнем да пишем код. Можем да разделим задачата на няколко подзадачи: 1. Да прочетем входните данни от конзолата за количеството зеленчуци и плодове, както и техните цени 2. Да извършим необходимите изчисления. В случая трябва да изчислим общия приход: това ще стане като съберем прихода от зеленчуците с този от плодовете. Колко струват зеленчуците и плодовете общо, ще разберем като умножим количеството им по цената на килограм. Входните ни данни са дадени в лева, а от нас се изисква да отпечатаме резултата в евро. По условие 1 евро е 1.94 лв, следователно след като сме намерили общия приход в лева, трябва да разделим резултата на 1.94; така ще получим исканата сума. 3. Да отпечатаме резултата на конзолата. 

След като сме изяснили идеята си за решаването на задачата, трябва да преценим какви типове данни ще използваме. Разглеждаме внимателно входните данни: за тях имаме две цели числа и две числа с плаваща запетая. Като изход се изисква да се отпечата едно число с плаваща запетая. Както вече знаем, в C# за целите числа е подходящо да използваме променливи от тип `int`, за реалните: `double`. С `Console.ReadLine()` четем стойностите от конзолата, а с `int.Parse()` и `double.Parse()` преобразуваме зададената стрингова стойност в съответно `int` и `double`.
