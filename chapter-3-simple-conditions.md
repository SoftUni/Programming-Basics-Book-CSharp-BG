...
Конструкцията `if` може да съдържа и `else` клауза с която да окажем конкретно действие в случай, че булевият израз, който е зададен в началото `if(булев израз)` върне отрицателен (`false`) резултат. Така построена **условна конструкция** наричаме `if-else` и поведението й е следното: ако резултатът от условието е **позитивен** - извършваме едни действия или когато е **негативен** - други. Форматът на конструкцията е:
