...
2. **Напишете кода** на програмата. Проверката дали дадено число е четно, може да се реализира с оператора `%`, който ще ни върне **остатъка при целочислено деление на 2** по следния начин: `var isEven = (num % 2 == 0)`.
