...
<table><tr><td><img src="assets/alert-icon.png" style="max-width:50px" /></td>
<td>Операторът <b>break</b> спира изпълнението на цикъла към момента, в който е извикан. Това значи, че текущата итерация няма да бъде завършена и съответно останалата част от кода в тялото на цикъла няма да се изпълни.</td>
</tr></table>
...
**Try...catch** е конструкция, която служи за проверка на обработваните данни и реагиране при **изключения**, **грешки** в тях.
...
**Условие:** Да се въведе **цяло** число n и да се пресметна n-тото число на Фибоначи

Решението можем да тестваме в judge системата: https://judge.softuni.bg/Contests/Practice/Index/156#11

Ето какви са нужните **стъпки**, за да може нашата програма да се изпълни коректно:

  1. Създаваме **променлива n**, на която присвояваме целочислена стойност от входа на конзолата.
  1. Създаваме променливите **f0** и **f1**, на които присвояваме стойност **1**, тъй като така започва редицата.
  1. Създаваме `for` цикъл, с условие **текущата стойност (i) < n - 1**.
  1. В **тялото на цикъла:**
      1. Създаваме **временна** променлива **fNext**, на която присвояваме следващото число в поредицата на Фибоначи.
      1. На **f0** присвояваме текущата стойност за **f1**.
...
**Условие:** Да се отпечатат числата 1…n в пирамида като в примерите:

|Вход      |Изход          |Вход      |Изход          |
|----------|---------------|----------|---------------|
|7         |1<br>2 3<br>4 5 6<br>7    |10    |1<br>2 3<br>4 5 6<br>7 8 9 10 |

Решението можем да тестваме в judge системата: https://judge.softuni.bg/Contests/Practice/Index/156#12

Ето какви са нужните **стъпки**, за да може нашата програма да се изпълни коректно:

1. Създаваме променлива **n**, на която присвояваме целочислена стойност от входа на конзолата.
1. Създаваме променлива **num** с начална стоност = 1. Тя ще пази броя на отпечатаните числа. При всяка итерация, ще я **увеличаваме**  с **1** и ще я принтираме.
1. Създаваме **външен** `for` цикъл, който ще отговаря за **редовете** в таблицата. Наименуваме променливата на цикъла **row** и ѝ задаваме начална стойност 0. За условие слагаме **row < n**. Размерът на стъпката е 1.
1. В тялото на цикъла:
    1. Създаваме **вътрешен** `for` цикъл, който ще отговаря за **колоните** в таблицата. Наименуваме променливата на цикъла **col** и ѝ задаваме начална стойност 0. За условие слагаме **col < row** (row = брой цифри на ред). Размерът на стъпката е 1.
    1. В тялото на вложения цикъл:
        1. Проверяваме дали **col > 1**, ако да – принтираме разстояние. Ако не правим тази проверка, а директно принтираме разстоянието, ще имаме ненужно такова в началото на всеки ред.
        1. **Отпечатваме** числото **num** в текущата клетка на таблицата и го **увеличаваме с 1**.
        1. Правим проверка за **num > n**. Ако num е по-голямо от n, **прекъсваме** въртенето на **вътрешния цикъл**.
...
**Условие:** Да се отпечатат числата 1…n в таблица като в примерите:

|Вход (n)|Изход|
|--------|-----|
|3       |1 2 3 <br/> 2 3 2 <br/> 3 2 1   |
|4       |1 2 3 4 <br/> 2 3 4 3 <br/> 3 4 3 2 <br/> 4 3 2 1 |

Решението можем да тестваме в judge системата: https://judge.softuni.bg/Contests/Practice/Index/156#13

Ето какви са нужните **стъпки**, за да може нашата програма да се изпълни коректно:

  1. Създаваме променлива **n**, на която присвояваме целочислена стойност от входа на конзолата.
  1. Създаваме `for` цикъл, който ще отговаря за редовете в таблицата. Наименуваме променливата на цикъла **row** и ѝ задаваме начална **стойност 0**. За условие слагаме **row < n**. Размерът на стъпката е 1.
  1. В **тялото на цикъла**:
      1. Създаваме вложен `for` цикъл, който ще отговаря за колоните в таблицата. Наименуваме променливата на цикъла **col** и ѝ задаваме начална **стойност 0**. За условие слагаме **col < n**. Размерът на стъпката е 1.
      1. В **тялото на вложения цикъл**:
          1. Създаваме променлива **num**, на която присвояваме резултата от **текущият ред + текущата колона + 1** (+ 1, тъй като започваме броенето от 0).
          1. Правим проверка за **num > n**. Ако num е **по-голямо** от n, присвояваме нова стойност на num равна на **два пъти n - текущата стойност за num**. Това правим с цел да не превишаваме **n**, в никоя от клетките на таблицата.
          1. Отпечатваме числото от текущата клетка на таблицата.
      1. Отпечатваме **празен ред**, за да преминем на следващия.
...
<img src="assets/Screenshot_28.png" style="transform: translate(-25%) scale(0.5);" />
