...
Стъпката е тази част от конструкцията на `for` цикъла, която указва с колко да се увеличи или намали текущата стойност за променливата на цикъла. Най-често стъпката е със **стойност = 1** и за удобство, вместо да пишем **i += 1**, можем да ползваме инкрементиращия оператор - **i++**. Ако решим стъпката ни да е **различна от 1**, то `for` цикълът би изглеждал по следния начин:

  ```cs
  // Ползваме 10 за стойност на стъпката.
...
Решението можем да тестваме в judge системата: https://judge.softuni.bg/Contests/154.
...
Нека видим какви са нужните стъпки, за да може нашата програма да се изпълни коректно:
  ...
  2. Създаваме `for` цикъл с **размер на стъпката = 3**:
  3. В **тялото на цикъла** отпечатваме стойността на текущата стъпка.
  
  ![Обяснение на стъпка във for цикъл](assets/chapter-7-images/for-loop-step-explanation.png)

TODO: Да се сложат поясненията за горният код.
