...
**i++/i--**. Ако искаме стъпката ни да е **различна от 1**, при увеличение използваме оператора **i += размера на стъпката**, а при намаляване **i -= размера на стъпката**. При стъпка = 10, цикълът би изглеждал по следния начин: 

![image](assets/chapter-7-images/step-explanation.png)

Следва поредица от примерни задачи, решението на които ще ни помогне да разберем по-добре употребата на **стъпката** в `for` цикъл.
...
Нека видим какви са нужните **стъпки**, за да може нашата програма да се изпълни коректно:
  ...
  ```cs
  // Създаваме променливата num и ѝ присвояваме стойност 1
  var num = 1;
...
// TODO: Пишем израз, който да валидира, че въведеното число е в диапазона [1...100].
while (условие)
{
...
// TODO: Създаваме променливите a и b, и им присвояваме целочислени стойности от входа на конзолата.
...
Един цикъл е безкраен когато повтаряме тялото му до безкрайност. Това правим, като му зададем условен израз, който **винаги** да е **true**. 
...
### Оператор break
Научихме, че безкрайният цикъл изпълнява определен код до безкрайност, но какво става, ако желаем в определен момент, при дадено условие, да излезем принудително от цикъла. Тук на помощ ни идва оператор **break**.
...
```cs
// Създаваме променливата n и ѝ присвояваме цяло число въведено от конзолата.
var n = int.Parse(Console.ReadLine());
// Създаваме начална булевата променлива isPrime с начална стойност true.
var isPrime = true;
// Създаваме `for` цикъл с начална стойност за променливата (i) = 2, текущата стойност < корен квадратен от n за условие и размер на стъпката = 1.
for (var i = 2; i <= Math.Sqrt(n); i++)
{
  // Проверяваме дали n разделено на i връща остатък 0.
  // Ако върне остатък 0, то значи делението е било успешно и числото не е просто.
  if (проверка за остатък) 
  {
    // TODO: Променяме стойността на isPrime на false.
    // TODO: Излизаме принудително от цикъла чрез оператор break.
  }
}
// След излизане от цикъла проверяваме дали числото се е оказало просто.
if (проверка дали числото е просто) 
{
  // TODO: Отпечатваме, че числото е просто.
}
else 
{
  // TODO: Отпечатваме, че числото не е просто.
}
```
...
```cs
// Създаваме променлива n с начална стойност 0.
var n = 0;
// Създаваме безкраен while цикъл
while (true)
{
  // TODO: На n присвояваме целочислена стойност от входа на конзолата.
  if (проверка за четност на n)
  {
    // TODO: излизаме от цикъла.
  }
  // TODO: Отпечатваме съобщение на конзолата, което ни известява, че числото не е четно.
}
// TODO: Отпечатваме четното число.
```
...
try
{
  // Обичайна програмна логика...
}
catch
{
  // Програмна логика, която се изпълнява при улавяне на изключение от кода в try { }
}
```
