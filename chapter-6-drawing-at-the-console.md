...
В този пример делението не е от 2 целочислени числа. "`f`" след число показва, че даденото число е от тип `float` (число с плаваща запетая). Резултатът от `3 / 2f` е `1.5f`. `Math.Cleiling(...)` закръгля делението нагоре. В нашият случай `1.5f` ще стане 2. `(int)` се изпозлва, за да може да трансформираме типа обратно в `Integer`
...
|Вход|Изход|
|---|---|
|9|<code>----\*----</code><br><code>---\*-\*---</code><br><code>--\*---\*--</code><br><code>-\*-----\*-</code><br><code>\*-------\*</code><br><code>-\*-----\*-</code><br><code>--\*---\*--</code><br><code>---\*-\*---</code><br><code>----\*----</code>|
...
* Рисуваме по конзолата левите **тиренца** (с дължина **`leftRight`**) и веднага след тях първата **звезда**.

![](assets/chapter-6-images/diamond/left-part-drawing.PNG)

* Ще изчислим разстоянието между двете **звезди**. Може да го изчислим като извадим от **n** дължината на външните **тиренца**, както и числото 2 (бройката на **звездите**, т.е. очертанията на диаманта). Резултата от тази разлика записваме в променлива **`mid`**. 
