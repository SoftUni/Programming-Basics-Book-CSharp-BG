...
| Вход | Изход | Вход | Изход | Вход | Изход | Вход | Изход | Вход | Изход |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 2<br>100<br>99 | 100 | 3<br>-10<br>20<br>-30 | 20 | 4<br>45<br>-20<br>7<br>99<br> | 99 | 1<br>999 | 999 | 2<br>-1<br>-2 | -1 |

Първо въвеждаме едно число **n** (броят числа, които предстои да бъдат въведени). Слагаме текущият максимум **max** да е прочетеното число. С помощта на `for` цикъл, който итерираме **n-1 пъти**, прочитаме по едно цяло число **num**. Ако прочетеното число **num** е по-голямо от текущия максимум **max**, запомняме **num** в **max**. На края в **max** трябва да се е запазило най-голямото число. Отпечатваме го на конзолата.
...
Да се напише програма, която въвежда **n** числа, проверява дали **сумата на числата на четни позиции** е равна на **сумата на числата на нечетни позиции**. При равенство печата **"Yes" + сумата**, иначе печата **"No" + разликата**. Разликата се изчислява по абсолютна стойност. Примери:

| Вход | Изход | Вход | Изход | Вход | Изход |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 4<br>10<br>50<br>60<br>20 | Yes<br>Sum = 70 | 4<br>3<br>5<br>1<br>-2 | No<br>Diff = 1 | 3<br>5<br>8<br>1 | No<br>Diff = 2 |

Въвеждаме числата едно по едно и изчисляваме двете **суми** (числа на **четни** позиции и числа на **нечетни** позиции). Както в предходната задача, изчисляваме абсолютната стойност на разликата и отпечатваме резултата (**"Yes" + сумата** при разлика 0 или **"No" + разликата** в противен случай).
...
Да се напише програма, която въвежда **текст** (стринг) и отпечатва **сумата на гласните букви** според таблицата по-долу:
...
Прочитаме входния текст **s**, нулираме сумата и завъртаме цикъл от **0** до **s.Length-1** (дължината на текста -1). Проверяваме всяка буква **s[i]** дали е гласна и съответно добавяме към сумата стойността й.
