...
Цикълът започва с **оператора `for`** и преминава през всички стойности за дадена променлива в даден интервал, например всички числа от 1 до 10 включително, и за всяка стойност изпълнява поредица команди.
...
Първо въвеждаме едно число **`n`** (броят числа, които предстои да бъдат въведени). Задаваме на текущия максимум **`max`** първоначална неутрална стойност, например **-10000000000000** (или **`int.MinValue`**). С помощта на **`for-цикъл`**, чрез който итерираме **n-1 пъти**, прочитаме по едно цяло число **`num`**. Ако прочетеното число **`num`** е по-голямо от текущия максимум **`max`**, присвояваме стойността на **`num`** в променливата **`max`**. На края в **max** трябва да се е запазило най-голямото число. Отпечатваме го на конзолата.
...
Изчислете **сумата** на всички елементи, намерете **най-големия** от тях и проверете търсеното условие.
...
Задачата обединява няколко предходни задачи: 

* намиране на **минимум**, **максимум** и **сума**, както и обработка на елементите от **четни и нечетни позиции**. Припомнете си ги.
...
* прочитаме входните числа **по двойки**. За всяка двойка пресмятаме **сумата**
*	докато четем входните двойки, за всяка двойка без първата, трябва да пресметнем **разликата с предходната**. За целта е необходимо да пазим в отделна променлива сумата на предходната двойка
* намираме **най-голямата разлика** между две двойки. Ако е **0**, печатаме **“Yes”** + стойността, в противен случай - **“No”** + разликата
...
Ще се отвори прозорец за търсене и инсталиране на **NuGet** пакети. Нека търсим пакети по ключова дума **“nakov”**. Ще излязат няколко пакета. От тях избираме пакет **“Nakov.TurtleGraphics”**. Натискаме [**Install**], за да го инсталирате към нашия C# проект:
 
![code](assets/chapter-5-images/13.Turtle-graphics-04.png)

Към нашия C# проект вече в включена външната библиотека **“Nakov.TurtleGraphics”**. Тя дефинира един клас **Turtle**, който представлява **костенурка за рисуване**. За да го използваме, трябва да добавим в C# кода за нашата форма (**Form1.cs**) следния код, най-отгоре в началото на файла:
