...
В тази глава ще разберем **какво е програмирането** в неговата същина. Ще се запознаем с идеята за **програмни езици** и ще разгледаме **средите за разработка на софтуер** (IDE) и как да работим с тях, в частност с **Visual Studio**. Ще напишем и изпълним **първата си програма** на програмния език C#, а след това ще се упражним с няколко задачи: ще създадем конзолна програма и графично приложение и уеб приложение. Ще се научим как да проверяваме за коректно кода си в **judge системата на СофтУни** и накрая ще видим какви са типичните грешки, които не трябва да допускаме при писането на код.
...
**Да програмираме** означава да даваме команди на компютъра какво да прави, например "*да изсвири някакъв звук*", "*да отпечата нещо на екрана*" или "*да умножи две числа*". Когато командите са няколко една след друга, те се наричат **компютърна програма**.

**Компютърните програми** представляват **поредица от команди**, подредени една след друга в определен ред. Командите се изписват на предварително определен **език за програмиране**, например C#, Java, JavaScript, Python, PHP, C, C++, Swift или друг.

За да пишем команди, трябва да знаем **синтаксиса и семантиката на езика**, с който ще работим, в нашия случай **C#**. За удобство при писането на команди и за улесняване на тяхното изпълнение и за други операции, свързани с програмирането, ни е необходима и **среда за разработка**, например Visual Studio.

### Езици за програмиране, компилатори и среди за разработка

Като старт, нека първо изясним какво точно представляват **средите за разработка**, **компилаторите** и **програмните езици**.

**Езикът за програмиране** е изкуствен език (синтаксис за изразяване), предназначен за **задаване на команди**, които искаме компютърът да прочете, обработи и изпълни. Чрез езиците за програмиране можем да създаваме програми, които да **контролират поведението на технологичния апарат**.

**Компилаторът** превежда кода от програмен език на **машинен код**, като за всяка от конструкциите избира подходящ, предварително подготвен фрагмент от машинен код. Заедно тези фрагменти съставят програмата в машинен вид, както я очаква микропроцесорът на нашия компютър. Междувременно **текстът на програмата бива проверяван за грешки**. След това всичко се компилира и програмата се изпълнява.

Някои езици за програмиране не използват компилатор, а се **интерпретират директно** от специализиран софтуер, наречен "интерпретатор". **Интерпретаторът** е "**програма за изпълняване на програми**", написани на някакъв език за програмиране. Езици като PHP, Python и JavaScript работят с интерпретатор и се изпълняват без да се компилират.

**Средата за програмиране** (Integrated Development Environment - **IDE**, интегрирана среда за разработка) е съвкупност от традиционни инструменти за разработване на всяка от функциите по създаване на едно приложение. В нея обикновено са интегрирани **езикът за програмиране**, **компилатор**, **редактор**, **изпълнителна система**, **система за поддържане на библиотеки**, както и други полезни добавки. Средите за програмиране са удобни за използване, защото съдържат всичко необходимо за разработката на програмата, без да се напуска средата на редактора. За програмиране на езика C# най-често се ползва средата за разработка **Visual Studio**, която се разработва и разпространява безплатно от Microsoft и може да се изтегли от: https://www.visualstudio.com/downloads/.
...
Съществуват различни видове **езици за програмиране**. С една част от тях могат да бъдат написани **самите инструкции**, които **управляват процесора**, или инструкции, които **управляват RAM паметта** на компютъра. Пример за такъв език е "**Assembler**". С друга част могат да бъдат създадени операционна система, драйвер за видеокарта и т.н.

Самият софтуер, който използваме ежедневно, като музикален плеър, видеоплеър, GPS програма и т.н., се пише на езици като C#, Java, Python, C, C++, JavaScript и др. 
...
C# е език, който е компилируем, което означава, че пишем самите инструкции. Именно тези инструкции, чрез помощна програма (компилатор), се преобразуват във файл, който може да се изпълнява (executable). За да пишем на език като **C#** ни трябва текстов редактор или среда за изпълнение, където да пишем кода и **.NET среда за изпълнение**.
...
Както вече споменахме, програмата е **последователност от команди**, иначе казано тя описва поредица от пресмятания, проверки, повторения и всякакви подобни операции, които целят постигане на някакъв желанат резултат.
...
Нашата първа програма ще е единична команда, която свири музикалната нота "ла" половин секунда:

![](assets/chapter-1-images/00.Programs-01.png)

След малко ще разберем как можем да изпълним тазо команда и да чуем звука от нотата, но засега нека само разгледаме какво представляват командите в програмирането. Да се запознаем с още няколко примера.
...
Да разгледаме още една проста програма, която прочита от потребителя някаква сума в лева (цяло число), конвертира я в евро (като я разделя на курса на еврото) и отпечатва полученият резултат. Това е програма от 3 поредни команди.
...
* JavaScript: може да се пише директно в конзолата на даден браузър с натискане на **[F12]**
