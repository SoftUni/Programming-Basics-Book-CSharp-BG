...
<td><strong>Централизация</strong> в задачите, които разглеждаме в настоящата книга изглежда на пръв поглед излишно, но този подход е от съществено значение при изграждането на мащабни приложения в реална работна среда и упражняването му в начален стадий на изучаване само ще подпомогне за усвояването на един качествен стил на програмиране..
</td>
</tr>
</table>
...
Обърнете внимание, че в условието е упоменато да закръглим броя на плочките получен от делението до по-високо цяло число, след което да прибавим **5**. Потърсете повече информация за дефинираната в езика `C#` функция `Math.Ceiling()` (https://msdn.microsoft.com/en-us/library/zx4t0t48(v=vs.110).aspx).
