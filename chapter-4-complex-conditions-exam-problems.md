...
задача 2 - не ми харесва това което си писал за инициализирането на променливи с някакви стойности. Реално единственото, което се спестява е един else и 2 скоби, а се разваля подредеността на кода. Също така във следващия if (за по-големия бюджет) отново ще трябва да се инициализират същите променливи, ако го строим аналогично на първия, което не мисля, че е добра практика.

задача 3 - на третата картинка има едни грешни скоби. Tрябва да е if (N2 == 0 && (nOperator.Equals("/") || nOperator.Equals("%"))). Това, което ти си написал винаги дава true ако оператора е %.
на 4тата картинка ти липсва затваряща скоба на else-a; 
реално задачата не е грешна, понеже имаш ограничения за входа, но ако се въведе оператор различен от тези които са в условието, изкарва някакъв си резултат, а не е хубаво така

задача 5 - switch-case-a има различна индентация на различните места.
накрая си ползвала decimal.Round - като цяло може да споменеш защо и какво е това. В крайна сметка се очаква, че повечето хора, които хващат книгата, тепърва виждат C#

Мисля, че трябваще част от задачите да са със замъглени картинки и читателя да се досеща сам как да ги решава (за тов не съм сигурен, дали е от теб, понеже нали щяха да оправят всички картинки). Ако това трябва да го правиш бих подходил със по праволинейни и прости решения, без пестене на код. Нека се научат да пишат първо по 16 иф-елс-а, а след това да се мъчат да пестят код.
