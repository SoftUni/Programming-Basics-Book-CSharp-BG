...
Повече за **бързите клавиши във Visual Studio** може да намерите тук: [https://msdn.microsoft.com/en-us/library/da5kh0wa.aspx](https://msdn.microsoft.com/en-us/library/da5kh0wa.aspx)
...
Ако искаме да закръглим до **2 десетични цифри** след десетичната запетая и третата цифра е по-малкa от 5, както в примера по-горе, то закръглянето е надолу, но ако третата цифра е 5 или по-голяма - закръглянето е нагоре, както е в примера по-долу:
...
В случая след числото добавяме **`:f2`**, което ще ограничи числото до 2 знака след десетичната запетая и ще работи като **`Math.Round()`**. Tрябва да имаме впредвид, че числото след буквата **"f"** означава до колко знака след десетичната запетая да е закръглено числото (т.е. може да е `f3` или `f5`).
...
**`if-конструкцията`** се състои от следните неща:
